 ...
 * points.c
 * ----------------------------
 * Defines functions for manipulating data for individual points, such as adding and deleting points.
 *
...
//Gets a particle's special value, falling back to default if needed
...
int getParticleSpecialVal(int tempParticle, int special)
{
    ...
    // If we didn't find the particle, fall back to default element
...
void clearSpecialVals(int tempParticle)
{
	...
	for (i = 0; i < MAX_SPECIALS; ++i)
    {
		 if (a_set[tempParticle] &&
		        		...
		        		!(a_element[tempParticle]->specials[i] == SPECIAL_BURN)) //||a_element[tempParticle]->specials[i] == SPECIAL_BREAK))
		        				{
