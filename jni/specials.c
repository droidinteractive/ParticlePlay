    ...
    for (diffX = -2; diffX <= 2; diffX++)
    {
        for (diffY = -2; diffY <= 2; diffY++)
        {
            if (tempX + diffX >= 0 && tempX + diffX < workWidth && tempY + diffY >= 0 && tempY + diffY < workHeight)
            {
                ...
                if (tempAllCoords != -1 && (a_element[tempAllCoords] == elements[REPLICATOR_ELEMENT] ||
                		a_element[tempAllCoords]->base == REPLICATOR_ELEMENT)) //There's a Replicator adjacent
                {
                ...
                else if (tempAllCoords == -1 && rand() % REPLICATOR_SPAWN_PROB == 0 && loq < MAX_POINTS - 1) //There's an empty spot
                {
...
void specialGrow(int particle)
{
    ...
    for (diffX = -1; diffX <= 1; diffX++)
    {
        for (diffY = -1; diffY <= 1; diffY++)
        {
            ...
            {
                tempAllCoords = allCoords[getIndex(tempX+diffX,tempY+diffY)];
                if (tempAllCoords != -1 && a_element[tempAllCoords]->index == getElementSpecialVal(a_element[particle], SPECIAL_GROW) && rand() % 40 == 0) // was 10, but 40 looks cool
                {
...
void specialWander(int particle)
{
    ...
    if (getParticleSpecialVal(particle, SPECIAL_ANT) != SPECIAL_VAL_UNSET)
    {
        return;
    }
...
void specialTunnel(int particle)
{
    ...
    {
        // Move the particle back to it's old location, to avoid collision velocities messing up tunneling
        ...
        if (tempAllCoords == -1 || a_element[tempAllCoords]->index != targetElementIndex)
        {
            // Go back to the unset state
            setParticleSpecialVal(particle, SPECIAL_TUNNEL, SPECIAL_VAL_UNSET);
            // Look for any particle back the way we came and (if there is one) tunnel into it
        ...
        else
        {
            ...
            // BIG TIME FIX -- FUCK YEAH! Now termites tunnel randomly
            // Go back to the unset state
            setParticleSpecialVal(particle, SPECIAL_TUNNEL, SPECIAL_VAL_UNSET);
...
// Need to figure out how to make some solid tunnels
void specialAnt(int particle)
{
    struct Element* tempElement = a_element[particle];
    struct Element* tempElement2 = a_element[8];
    int targetElementIndex = getElementSpecialVal(tempElement, SPECIAL_ANT);
    int tunnelElement = 3;
    int state = getParticleSpecialVal(particle, SPECIAL_ANT);
    ...
    if (state == SPECIAL_VAL_UNSET)
    {
        ...
        if (tempAllCoords != -1 && a_element[tempAllCoords]->index == targetElementIndex)
        {
            ...
            a_x[particle] = curX + diffX;
            a_y[particle] = curY + diffY;
            setParticleSpecialVal(particle, SPECIAL_ANT, randomDir);
            ...
            if (curX+2*diffX >= 0 && curX+2*diffX < workWidth && curY+2*diffY >= 0 && curY+2*diffY < workHeight)
            {
                if (allCoords[getIndex(curX+2*diffX, curY+2*diffY)] == -1)
                {
                    createPoint(curX+2*diffX, curY+2*diffY, elements[tunnelElement]);
                }
                if (allCoords[getIndex(curX+2*diffX, curY+diffY)] == -1)
                {
                    createPoint(curX+2*diffX, curY+diffY, elements[tunnelElement]);
                }
                if (allCoords[getIndex(curX+diffX, curY+2*diffY)] == -1)
                {
                    createPoint(curX+diffX, curY+2*diffY, elements[tunnelElement]);
                }
            }
        }
    }
    ...
    else
    {
        ...
        {
            // Go back to the unset state
            setParticleSpecialVal(particle, SPECIAL_ANT, SPECIAL_VAL_UNSET);
        ...
        int tempAllCoords = allCoords[getIndex(curX+diffX, curY+diffY)];
        if (tempAllCoords == -1 || a_element[tempAllCoords]->index != tunnelElement)
        {
            // Go back to the unset state
            setParticleSpecialVal(particle, SPECIAL_ANT, SPECIAL_VAL_UNSET);
            // Look for any particle back the way we came and (if there is one) tunnel into it
