 ...
 * update.c
 * -----------------------------------
 * Defines the function UpdateView(), which is called every frame to update all the particles' positions.
 *
...
int updateSpecials(int index)
{
    ...
    if (!tempElement)
    {
        //__android_log_write(ANDROID_LOG_ERROR, "Particle Play", "Null particle in specials");
...
void UpdateView(void)
{
    ...
    if (play)
    {
        ...
        if (rand()%100 == 1) // Was 1000, testing 100 temporarily
        {
            randOffset = rand();
        }
        ...
        for (tempParticle = 0; tempParticle < MAX_POINTS; ++tempParticle)
        {
            ...
            if (a_set[tempParticle])// && tempParticle->frozen < 4)
            {
                ...
                //Inertia immovable object still need to deal with velocities
                else
                {
                    //Reduce velocities
                    if(*tempXVel < 0)
